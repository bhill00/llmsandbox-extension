...
export class ChatViewProvider implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(webviewView: vscode.WebviewView) {
    ...
    this.postMessage({
      ...
      apiKeySet: !!config.get<string>("apiKey", ""),
    });
  ...
  private async handleChat(text: string) {
    ...
    const autoInclude = vscode.workspace.getConfiguration("llmsandbox").get<boolean>("autoIncludeActiveFile", true);
    ...
    try {
      const body: Record<string, unknown> = { message: resolved };
      if (activeFile) {
      ...
      const fileOriginals: Record<string, string> = {};
  ...
  private async saveSettingsAndStart(apiUrl: string, apiKey: string) {
    const config = vscode.workspace.getConfiguration("llmsandbox");
